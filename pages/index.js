...
const SCROLL_SEPOLIA_CONTRACT_ADDRESS =
  process.env.NEXT_PUBLIC_SCROLL_SEPOLIA_CONTRACT_ADDRESS;
const ETH_SEPOLIA_CONTRACT_ADDRESS =
  process.env.NEXT_PUBLIC_ETH_SEPOLIA_CONTRACT_ADDRESS;
const ETH_SEPOLIA_RPC_URL = process.env.NEXT_PUBLIC_ETH_SEPOLIA_RPC_URL;
...
export default function Home() {
  ...
  const { data: useContractWriteUSDCData, write: approveWrite } =
    useContractWrite({
      address: "0x254d06f33bDc5b8ee05b2ea472107E300226659A", // Address of the aUSDC contract on Scroll_Sepolia
      abi: erc20ABI,
      functionName: "approve",
      args: [
        SCROLL_SEPOLIA_CONTRACT_ADDRESS,
  ...
  const {
    ...
    args: [address, SCROLL_SEPOLIA_CONTRACT_ADDRESS],
  });
  ...
  const { data: useContractWriteData, write } = useContractWrite({
    address: SCROLL_SEPOLIA_CONTRACT_ADDRESS,
    abi: BountyContract.abi,
    functionName: "sendToMany",
    args: [
      "ethereum-sepolia",
      ETH_SEPOLIA_CONTRACT_ADDRESS,
      Addresses.split(","),
      "aUSDC",
      ethers.utils.parseUnits(amount.toString(), 6),
    ],
      ...
      <main className="flex-grow flex flex-col items-center justify-center">
        ...
        >
          <h1 className="text-5xl font-bold mb-10 text-center text-gray-800">
            Cross-chain Bounty dApp on Scroll with{" "}
        ...
        <div className="flex justify-center">
          <div className="max-w-2xl bg-white border border-gray-200 rounded-lg shadow-md p-8 m-4">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">
              Send Bounty 💸 Scroll to ETH Sepolia
            </h2>
            <div className="mb-6">
